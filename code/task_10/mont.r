...
run <- function(g, n_traits, n_features, p_dictator = 0) {
    ...
    while (active_pairs != 0 && i < max_iter) {
	  ...
	  random_node <- sample(V(g), 1)
  	  dictator <- sample(dictators, 1)[[1]]
  	  diff_features <- which(V(g)$label[[random_node]] != dictator)
  		if (length(diff_features) > 0) {
			feature_to_change <- sample(diff_features, 1)
		        V(g)$label[[random_node]][feature_to_change] <- dictator[feature_to_change]
  		}
       }	
...
dictators <- list(sample(1:n_traits, n_features, replace=TRUE))
while (length(dictators) < 3) {
  new_d <- sample(1:n_traits, n_features, replace=TRUE)
  if (all(sapply(dictators, function(d) sum(d == new_d)) < n_features)) {
    dictators[[length(dictators) + 1]] <- new_d
  }
}
...
write.csv(all_results, file = "all_results_dictator_mont_e5.csv", row.names = FALSE)
