...
run <- function(g, n_traits, n_features, p_dictator = 0) {
    ...
    while (active_pairs != 0 && i < max_iter) {
      ...
      if (runif(1) < p_dictator) {
        ...
        sim_dictator <- sum(V(g)$label[[random_node]] == dictator) / n_features
        if (sim_dictator != 1 && runif(1) < sim_dictator) {
...
write.csv(all_results, file = "all_results_persuasor_e4.csv", row.names = FALSE)
